...
/**
 * Opmode for configuring the Brushland Labs Color Rangefinder.
 * Fully configures sensor, from now on can use:
 * DigitalChannel pin0 = hardwareMap.digitalChannel.get("digital0");
 * then pin0.getState()
 */
@TeleOp(name="Switch Brushland from I2c to Digital", group="z")
public class DigitalColorSensorConfig extends OpMode {
    ...
    public void init() {
        ...
        /// this should configure permanently until the sensor gets reset?
        // Pin0 = Green
        crf.setPin0Digital(ColorRangefinder.DigitalMode.HSV,
                100 / 360.0 * 255,
                140 / 360.0 * 255); // green 100°-140° Hue TODO tune
        crf.setPin0DigitalMaxDistance(ColorRangefinder.DigitalMode.HSV, 20); // 20mm or closer TODO tune

        // Pin1 = Purple
        crf.setPin1Digital(ColorRangefinder.DigitalMode.HSV,
                270 / 360.0 * 255,
                310 / 360.0 * 255); // purple 270°-310° Hue TODO tune
        crf.setPin1DigitalMaxDistance(ColorRangefinder.DigitalMode.HSV, 20); // 20mm or closer TODO tune
    }

    @Override
    public void loop() {
        /// Configuring process:
        /// 1. Plug Brushlands color sensor into digital 0-1
        /// 2. Init and run this opmode
        /// 3. Use telemetry so make sure code works
        /// 3a. If code no work, change Hue range and distance, push, go back to step 2
        /// 4. Repeat 1-3 for the 2 other color sensors
        /// *If change distance threshold at any time, have to reconfigure each of the three sensors

        boolean channel0 = hardwareMap.digitalChannel.get("pin0").getState(); // teal
        boolean channel1 = hardwareMap.digitalChannel.get("pin1").getState(); // purple

        String detectedColor = "";

        if (channel0 && !channel1) {
            detectedColor = "Green";
        } else if (channel1 && !channel0) {
            detectedColor = "Purple";
        } else {
            detectedColor = "Empty";
        }
        telemetry.addData("channel0 bool: ", channel0);
        telemetry.addData("channel1 bool: ", channel1);
