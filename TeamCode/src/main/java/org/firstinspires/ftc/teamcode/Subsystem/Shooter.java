...
public class Shooter {
    ...
    public static double MINIMUM_DISTANCE = 48; // TODO TODO TODO
    ...
    public volatile double targetHoodAngle = 40;
...
//        opmode.telemetry.addData("calculated RPM", distanceToRPM(dist));
    ...
    public void operateShootWhileMove(boolean vinWantsToShoot, double adjustedDist, boolean cyclingFarZone) {
        double currentRPM = getCurrentRPM();
        if (autoRPMmode) {
    ...
    public void operateShootWhileMoveSimple(double x_dist, double y_dist, boolean vinWantsToShoot, boolean cyclingFarZone) {
        double currentRPM = getCurrentRPM();
        // calculate distance
        double dist = Math.hypot(x_dist, y_dist);
...
//        targetAngleDegrees = Math.max(MIN_ANGLE_DEG, Math.min(MAX_ANGLE_DEG, targetAngleDegrees));
    ...
    public double distanceToRPM(double distance) {
        return (a1 / (1 + Math.exp(-(b1 * distance - c1)))) + MINIMUM_RPM;//regression equation
    }
